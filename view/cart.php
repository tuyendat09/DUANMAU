...
foreach ($_SESSION['cart'] as $item) {
    ...
    <td>'.$i.'</td>
    <td class="w-28"> 
    <img src="view/layout/images/'.$img.'" alt="" width="25"> 
    </td>
    <td>'.$name.'</td>
    <td>'.number_format($price,3).' đ</td>
    <td>'.$quantity.'</td>
    <td>'.number_format($price,3).' đ</td>
    <td><a href="'.$link_delete.'" class="text-second hover:text-black cursor-pointer transition duration-400">Xóa</a></td>
    </tr>
    ';
    $i++;
}
...
<section class="containerfull mt-40 hidden" style="margin-bottom: 225px;">
        <div class="container">
            ...
            <a href="index.php?pg=viewcart&del=1" class=" mt-4 p-4 border border-second text-second
        ...
        <div class="col3">
            <h2>ĐƠN HÀNG</h2>
            <div class="total">
                <h3>Tổng: <?=number_format($total_cart,3) . " đ"?></h3>
            </div>
            ...
            <div class="total">
                <h3><?=number_format($lastPrice,3) . " đ"?></h3>
            </div>
            <button class="mt-4 border-2 border-second p-4 hover:bg-second transition duration-300 hover:text-main text-second" type="submit">Thanh toán</button>
            <?php
